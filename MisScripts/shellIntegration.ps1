...
function Global:Prompt() {
    ...
    if ($Global:__VSCodeState.LastHistoryId -ne -1 -and ($Global:__VSCodeState.HasPSReadLine -eq $false -or $Global:__VSCodeState.IsInExecution -eq $true)) {
        ...
        # Ensure all command output is flushed before sending CommandEnd sequence
        # PowerShell's output can be buffered in multiple layers:
        # 1. Console.Out buffer (stdout)
        # 2. PowerShell host's UI output buffer
        # 3. ConPTY buffer (on Windows)
        # We need to flush all layers to prevent OSC 633;D arriving before actual output
        # This can be enabled via WINDSURF_ENABLE_OUTPUT_FLUSH_DELAY for environments that need it
        if ($env:WINDSURF_ENABLE_OUTPUT_FLUSH_DELAY) {
            try {
                [Console]::Out.Flush()
                [Console]::Error.Flush()
                # On Windows with ConPTY, give the host UI a moment to finish processing output
                # This race condition doesn't occur on Unix-based systems
                if ($IsWindows -and $Host.UI -and $Host.UI.SupportsVirtualTerminal) {
