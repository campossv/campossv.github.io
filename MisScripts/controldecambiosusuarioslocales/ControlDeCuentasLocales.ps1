...
function CrearCuerpoCorreo($event, $eventDescription, $cambiosDetectados, $datosActuales) {
    $formattedMessage = $event.Message -replace "\r\n", "<br>" -replace "&", "&amp;" -replace "<", "&lt;" -replace ">", "&gt;" -replace "&lt;br&gt;", "<br>"
    
    # Crear tabla de resumen de grupos y miembros actuales
    $resumenGrupos = ""
    foreach ($dato in $datosActuales) {
        $resumenGrupos += "<tr>
            <td>$($dato.Grupo)</td>
            <td>$($dato.Miembro)</td>
            <td>$($dato.NombreLimpio)</td>
            <td>$($dato.UltimoLogon)</td>
            <td>$($dato.CuentaActiva)</td>
            <td>$($dato.CuentaDeServicio)</td>
    ...
    <body>
        ...
        <table>
            ...
            <tr><th>Detalles del Evento</th><td>$formattedMessage</td></tr>
        </table>
        <h3>Cambios Detectados:</h3>
        <table>
            <tr><th>Grupo</th><th>Miembro</th><th>Tipo de Cambio</th><th>Detalles</th></tr>
            $cambiosDetectados
        </table>
        <h3>Resumen de Grupos y Miembros actuales:</h3>
        <table>
            <tr>
                <th>Grupo</th>
                <th>Miembro</th>
    ...
    if (Test-Path $NombreArchivo) {
        ...
        if ($Cambios) {
            ...
            foreach ($Cambio in $Cambios) {
                ...
                $cambiosDetectados += "<tr><td>$($Cambio.Grupo)</td><td>$($Cambio.NombreLimpio)</td><td>$Indicador</td><td>CuentaActiva=$($Cambio.CuentaActiva), CuentaDeServicio=$($Cambio.CuentaDeServicio), TipoCuenta=$($Cambio.TipoCuenta)</td></tr>"
            ...
            $cambiosDetectados = "<tr><td colspan='4'>No se detectaron cambios significativos desde la última revisión.</td></tr>"
        }
    } else {
        Escribe-log "Primera ejecución. No hay archivo anterior para comparar" -Level "INFO"
        $cambiosDetectados = "<tr><td colspan='4'>Primera ejecución. No hay archivo anterior para comparar.</td></tr>"
    }
...
try {
    ...
    Escribe-log "Error inesperado en el script: $_" -Level "ERROR"
}