...
function Write-Log {
    ...
    $logPath = "C:\Windows\PowerShell\ConfigLogs"
    $logFile = Join-Path $logPath "EmailConfig_$(Get-Date -Format 'yyyyMM').log"
...
$botonProbar.Add_Click({
    ...
    try {Write-Log "Iniciando prueba de conexión con configuración: Servidor=$servidorSmtp, Puerto=$puerto, SSL=$(if($botonSSL.Checked){'Habilitado'}else{'Deshabilitado'})"
    ...
    catch {Write-Log "Error en prueba de conexión: $($_.Exception.Message)" -Level "Error"
...
$botonConfigurar.Add_Click({
    ...
    Move-Item -Path .\ControlDeCuentasLocales.ps1 -Destination C:\Windows\PowerShell\
    ...
    Write-Log "Guardando nueva configuración: Servidor=$($configuracion.Servidor), Puerto=$($configuracion.Puerto), Protocolo=$($configuracion.Protocolo)"
    ...
    try {
        ...
        Write-Log "Error al guardar configuración: $($_.Exception.Message)" -Level "Error"
    ...
    # Lista de archivos XML a importar
    $tareasXml = @(
        "ControlCambioDeGrupo.xml",
        "CuentaCreada.xml",
        "Cuentadeusuariofuehabilitada.xml",
        "Cuentadeusuariofuedeshabilitada.xml",
        "CuentaEliminada.xml",
        "CuentaRemovidaDeGrupo.xml",
        "GrupoDeSeguridadCreado.xml",
        "GrupoDeSeguridadEliminado.xml"
    )
    
    $usuarioActual = [System.Security.Principal.WindowsIdentity]::GetCurrent().Name
    
    foreach ($xmlFile in $tareasXml) {
        Write-Log "Intentando importar tarea desde $xmlFile"
        
        if (-not (Test-Path -Path $xmlFile)) {
            Write-Log "No se encontró el archivo: $xmlFile" -Level "Error"
            [System.Windows.Forms.MessageBox]::Show("No se encontró el archivo: $xmlFile", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            continue
        }
        
        try {
            $xmlTarea = [xml](Get-Content $xmlFile -Raw)
            
            # Actualizar usuario en XML
            $nodoPrincipal = $xmlTarea.Task.Principals.Principal
            $nodoPrincipal.UserId = $usuarioActual
            $nodoPrincipal.LogonType = 'S4U'
            
            # Obtener nombre de la tarea del archivo
            $nombreTarea = [System.IO.Path]::GetFileNameWithoutExtension($xmlFile)
            
            # Registrar tarea
            $null = Register-ScheduledTask -TaskName $nombreTarea -Xml $xmlTarea.OuterXml -Force
            Write-Log "Tarea $nombreTarea importada exitosamente"
            
            # Eliminar archivo XML después de importar
            Remove-Item -Path $xmlFile
        }
        catch {
            Write-Log "Error al importar tarea $($xmlFile): $($_.Exception.Message)" -Level "Error"
            [System.Windows.Forms.MessageBox]::Show("Error al importar tarea $($xmlFile): $($_.Exception.Message)", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
        }
    }
    
    # Ejecutar ControlDeCuentasLocales.ps1 por primera vez
    try {
        Write-Log "Ejecutando ControlDeCuentasLocales.ps1 por primera vez"
        Start-Process powershell.exe -ArgumentList "-ExecutionPolicy Bypass -File C:\Windows\PowerShell\ControlDeCuentasLocales.ps1" -Wait
        Write-Log "Ejecución inicial completada exitosamente"
        ...
        Write-Log "Error en la ejecución inicial: $($_.Exception.Message)" -Level "Error"
        [System.Windows.Forms.MessageBox]::Show("Error en la ejecución inicial: $($_.Exception.Message)", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
    }
})
