...
function Buscar-Evento {
    ...
    try {
        $events = Get-WinEvent -FilterHashtable @{
            LogName = $LogName
            StartTime = $StartTime
        } -ErrorAction Stop | Where-Object { $_.Message -like "*$SearchString*" }
        return $events
    }
    catch {
        return $null
...
$buttonBuscar.Add_Click({
    ...
    $dias = [int]$textBoxDias.Text
    
    if ([string]::IsNullOrWhiteSpace($palabra)) {
        [System.Windows.Forms.MessageBox]::Show("Por favor, ingrese una palabra para buscar.", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
        return
    }
    
    if ($logsSeleccionados.Count -eq 0) {
        [System.Windows.Forms.MessageBox]::Show("Por favor, seleccione al menos un log.", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
        return
    }
    ...
    try {
        ...
        foreach ($log in $logsSeleccionados) {
            ...
            if ($eventosLog -ne $null) {
                ...
                $errores += "No se pudo buscar en el log '$log'. Puede que no tenga permisos o que el log esté vacío."
            }
        }
        
        if ($eventos.Count -eq 0 -and $errores.Count -eq 0) {
            [System.Windows.Forms.MessageBox]::Show("No se encontraron coincidencias.", "Resultado", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
        } else {
            $mensaje = ""
            if ($eventos.Count -gt 0) {
                $mensaje += "Se encontraron $($eventos.Count) coincidencias:`n`n"
                foreach ($evento in $eventos | Sort-Object TimeCreated -Descending) {
                    $mensaje += "Log: $($evento.LogName)`n"
                    $mensaje += "ID: $($evento.Id)`n"
                    $mensaje += "Fecha: $($evento.TimeCreated)`n"
                    $mensaje += "Mensaje: $($evento.Message)`n"
                    $mensaje += "-" * 50 + "`n`n"
                }
            }
            if ($errores.Count -gt 0) {
                $mensaje += "`nAdvertencias:`n"
                $mensaje += $errores -join "`n"
            }
            ...
            $textBoxResultados.Size = New-Object System.Drawing.Size(780, 550)
            $textBoxResultados.Location = New-Object System.Drawing.Point(10, 10)
            $textBoxResultados.Font = New-Object System.Drawing.Font("Consolas", 10)
            ...
            $buttonExportCSV.Add_Click({
                ...
                $DialogoGuardar.Title = "Guardar resultados como CSV"
                $DialogoGuardar.ShowDialog()

                if ($DialogoGuardar.FileName -ne "") {
                    $csvData = $eventos | ForEach-Object {
                        [PSCustomObject]@{
                            Log = $_.LogName
                            ID = $_.Id
                            Fecha = $_.TimeCreated
                            Mensaje = $_.Message -replace "`r`n", " " -replace "`n", " "
                        }
                    }
                    $csvData | Export-Csv -Path $DialogoGuardar.FileName -NoTypeInformation -Encoding UTF8
                    [System.Windows.Forms.MessageBox]::Show("Resultados exportados a CSV exitosamente.", "Éxito", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
                }
            })

            # Crear botón para exportar a JSON
            ...
            $resultForm.Controls.Add($buttonExportJSON)
            $resultForm.ShowDialog()
        }
    } catch {
        [System.Windows.Forms.MessageBox]::Show("Error general al buscar en los logs: $_", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
    }
})
